(() => {
  ...
  if (loader) {
    ...
    const words = ['Detect', 'Classify', 'Identify'];
  ...
  if (commandTriggers.length) {
    ...
    let items = [
      { title: 'Home', detail: 'Jinhao Wan — RF research portfolio', href: '/', type: 'Page' },
      { title: 'Research Atlas', detail: 'Four connected directions across the RF spectrum', href: '/research/', type: 'Page' },
      { title: 'Publications Library', detail: 'Search, filter and copy BibTeX', href: '/publications/', type: 'Page' },
      { title: 'Projects', detail: 'Methods, datasets and captures', href: '/projects/', type: 'Page' },
      { title: 'Interactive Explorer', detail: 'Compare four RF signal views in one responsive scene', href: '/explorer/', type: 'Explorer' }
    ];
    let visible = [];
    let activeIndex = 0;
    ...
    const renderResults = (query = '') => {
      ...
      if (!visible.length) {
        results.innerHTML = '<div class="command-empty">No matching work. Try a topic such as “modulation”, “UAV detection” or “RF fingerprinting”.</div>';
        return;
      }
