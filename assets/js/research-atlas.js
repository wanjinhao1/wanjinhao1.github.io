(() => {
  ...
  const edges = [
    ['dl-rf','signal-id'],['dl-rf','uav-detection'],['dl-rf','robust-recognition'],
    ['signal-id','uav-detection'],['signal-id','robust-recognition'],['uav-detection','robust-recognition']
  ];
  ...
  fetch('/assets/data/research.json')
    ...
    .then((data) => {
      ...
      nodes.innerHTML = data.domains.map((domain) => `<button class="atlas-node${domain.id === 'dl-rf' ? ' atlas-core active' : ''}" type="button" style="left:${domain.position.x}%;top:${domain.position.y}%" data-atlas-node="${escapeHtml(domain.id)}" aria-pressed="${domain.id === 'dl-rf'}"><b>${escapeHtml(domain.number)} · ${escapeHtml(domain.kicker)}</b><span>${escapeHtml(domain.title)}</span></button>`).join('');
